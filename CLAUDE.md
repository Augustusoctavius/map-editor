...
```
iso.js → catalog.js → catalog2.js → symbols.js → history.js → layers.js → canvas.js → tools.js → export.js → ui.js → app.js
```
...
- **`symbols.js`** — `Sym`, the flat (non-isometric) symbol index (~200+ ink-style symbols as `{d, f, s, lw, tr}` Path2D parts in a 0–100 coordinate space centered at (50,50)), plus custom PNG symbol upload/registration. The same path data draws to canvas via `Path2D` and is written verbatim as `<path>` in SVG export — SVG output is真 vector, not embedded bitmap.
