...
Wayborne Map Editor (aka "Cartographer") — a fantasy/medieval map editor. Vanilla JS + Canvas 2D only: no framework, no build step, no bundler, no npm dependencies, no CDN assets. Everything (608 symbols, 20 terrain types, isometric buildings) is generated in code, not loaded from files.
